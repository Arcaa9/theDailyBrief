...
export const handler = async (event) => {
    ...
    try {
        ...
        // Get all users
        const allUsers = await dynamoClient.send(new ScanCommand({
        ...
        for (const user of allUsers.Items) {
            ...
            const phone = user.phoneNumber?.S;
            ...
            await sqsClient.send(new SendMessageCommand({
                QueueUrl: QUEUE_URL,
                MessageBody: JSON.stringify({ emailId, userId, email, phone, presignedUrl, emailStatus: 'Pending' })
            }));

            console.log("Queued user", email);
        };

        console.log(`Queued ${allUsers.Items.length} users`);
    } catch (error) {
        console.log("Error while sending queue", error);
        throw error;
    }
};
